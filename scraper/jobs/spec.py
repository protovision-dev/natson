...
class Job:
    ...
    def from_cli(cls, args: argparse.Namespace) -> "Job":
        ...
        return cls(
            ...
            compset_id=int(args.compset_id) if args.compset_id is not None else int(u["compset_id"]),
            mealtype=int(args.mealtype) if args.mealtype is not None else int(u["mealtype"]),
            membershiptype=int(args.membershiptype) if args.membershiptype is not None else int(u["membershiptype"]),
...
def add_cli_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--hotels", required=True,
        help="Comma-separated hotel_ids, `portfolio`, or file:path.json")
    ...
    refresh_group.add_argument("--refresh", dest="refresh", action="store_true", default=None,
    ...
    refresh_group.add_argument("--refresh-only", dest="refresh_only", action="store_true", default=False,
        help="Trigger refresh but skip the rate fetch (warm-up mode)")
    parser.add_argument("--job-id", dest="job_id", default=None,
        help="Override job_id; default is timestamp-uuid6")
