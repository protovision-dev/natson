...
cookies = [
    {"name": c["name"], "value": c["value"],
     "domain": "app.mylighthouse.com", "path": "/", "secure": True, "sameSite": "None"}
...
refresh_candidates = re.findall(
    r'<(?:button|a|div|span)[^>]*(?:refresh|Refresh|resync|Resync|update|Update)[^>]*>',
    html, re.I
)
...
data_attrs = re.findall(r'data-(?:event-id|testid|action)="[^"]*(?:refresh|update|resync)[^"]*"', html, re.I)
...
sess.headers.update({
    ...
    "Referer": PAGE,
    "X-CSRFToken": next((c["value"] for c in SESSION["cookies"] if c["name"] == "csrftoken"), ""),
})
...
probes = [
    ...
    ("POST", "/apigateway/v1/app/shops/trigger/", {"hotel_id": "345062", "subscription_id": "345062"}),
]

for method, path, body in probes:
    try:
        r = sess.request(method, f"https://app.mylighthouse.com{path}",
                         json=body, timeout=15)
    except Exception as e:
        print(f"  {method:4} {path:<50}  ERR {e}")
        continue
    ct = r.headers.get("content-type", "")
    preview = (r.text[:200] if "json" in ct or len(r.text) < 500 else f"<{ct} {len(r.text)}B>").replace("\n", " ")
...
(OUT / "refresh_discovery.json").write_text(json.dumps({
    "baseline_xhrs": [{"url": x["url"], "status": x["status"], "method": x["method"]}
                       for x in baseline_xhrs],
