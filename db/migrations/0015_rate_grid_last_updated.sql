-- Extend v_rate_grid_latest with `observation_ts` (our scrape timestamp)
-- and `extract_datetime` (Lighthouse's "last shopped" timestamp).
-- Used by the Competitor legend to surface a Last Updated column.
...
SELECT
    ...
    ro.message,
    ro.observation_date,
    ro.observation_ts,         -- our scrape timestamp (set by run_job.py)
    ro.extract_datetime        -- Lighthouse's "last shopped" on the OTA
